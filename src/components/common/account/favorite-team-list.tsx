...
export default function FavoriteTeamList() {
	const [teamList, setTeamList] = useState<(TeamDto | null)[]>([
	...
	const handleAddButtonClick = () => {
		setTeamList([...teamList, null]);
		setSelectedTeamIndex(teamList.length);
	};
	...
	const handleXButtonClick = (e: React.MouseEvent, index: number) => {
		...
		// 모두 삭제 시 teamList를 [null]로 설정하여 추가 버튼이 생성되지 않도록 함
		const newTeamList = teamList.filter((_, i) => i !== index);
		setTeamList(newTeamList.length ? newTeamList : [null]);

		// 현재 선택된 팀을 삭제할 경우 첫 번째 요소나 마지막 요소를 active
		if (selectedTeamIndex === index) {
			const newSelectedTeamIndex = index === 0 ? 0 : newTeamList.length - 1;
			setSelectedTeamIndex(newSelectedTeamIndex);
		} else {
			// 그 외 기존 요소에 대한 active 유지
			const selectedTeam = teamList[selectedTeamIndex];
			const newSelectedTeamIndex = newTeamList.indexOf(selectedTeam);
			setSelectedTeamIndex(newSelectedTeamIndex);
		}
	};
	...
	return (
		<div className="grid grid-cols-3 gap-2.5 items-end">
			...
			{teamList.at(-1) !== null && teamList.length < 3 && (
